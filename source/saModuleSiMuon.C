// $Id: saModuleSiMuon.C,v 1.4 2015/09/03 20:45:38 yuhw Exp $                                                                                             

/*!
 * \file saModuleSiMuon.C
 * \author Landiao Liu <liulandiao@gmail.com>
...
// SiMuon 
saModuleSiMuon::saModuleSiMuon(const std::string &name, bool doControlData) :
	saModuleBase(name),                                     //
...
saModuleSiMuon::~saModuleSiMuon()
{

}

//! global initialization
int saModuleSiMuon::init(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
	//set_fitparam(2);
	...
	{
		cout << "saModuleSiMuon::Init failed to create saEventProperty"
			<< endl;
		return ABORTRUN;
	}
...
//! Run initialization
int saModuleSiMuon::init_run(PHCompositeNode *topNode,
...
//! event method
int saModuleSiMuon::event(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
	DiMuonContainer* dimuons = findNode::getClass<DiMuonContainer>(topNode,
			"DiMuonContainer");
	if (!dimuons)
	{
		cout << "saModuleSiMuon:: DiMuonContainer - ERROR - not in Node Tree"
			<< endl;
		return ABORTRUN;
	}
	...
	if (!ep)
	{
		cout << "saModuleSiMuon::event - ERROR  - Cannot find EventProperty node in Top Node"
			<< endl;
		return ABORTRUN;
	}
	...
	if (Verbosity() >= 2)
	{
		cout << "saModuleSiMuon::event - INFO  - ";
		ep->identify();
	}
	...
	if (!flag)
	{
		cout << "saModuleSiMuon::event - DimuonFlag not in Node Tree"
			<< endl;
		return ABORTRUN;
	}
	...
	if (!triglv1)
	{
		cout << "saModuleSiMuon:: TrigLvl1 - ERROR - not in Node Tree"
			<< endl;
		return ABORTRUN;
	}
...
//! global termination
int saModuleSiMuon::end(PHCompositeNode *topNode, sa_hist_mangager_ptr hm)
{
...
//! 
bool saModuleSiMuon::pass_dimuon_cut(const DiMuon *dimuon)
{
...
bool saModuleSiMuon::set_fitparam(int nSigma)
{
...
TH1 *saModuleSiMuon::makeTHist_pT_phi(
		std::string name,
		std::string title
		)
{
...
saHist *saModuleSiMuon::makesaHist_pT_phi(
		sa_hist_mangager_ptr hm,
		TH1 *h
		)
{
